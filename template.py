...
def compare_models(prompt: str) -> dict:
    ...
    gp4o_text, gp4o_latency = call_openai(prompt)
    mini_text, mini_latency = call_openai_mini(prompt)
    cost = (len(gp4o_text.split()) / 0.75) / 1000 \
        * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]

    return {
        "gpt4o_response": gp4o_text,
        "mini_response": mini_text,
        "gpt4o_latency": gp4o_latency,
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        stream = client.chat.completions.create(
            model=OPENAI_MODEL,
            messages=messages,
            stream=True #Khi có stream=True, API không trả về một response duy nhất mà trả về một dãy chunk, mỗi chunk chứa vài chữ.
        )
        ...
        #delta.content: phần chữ mới trong chunk, khi streaming, DL sẽ nằm ở .delta.content, không phải .message.content như part 1.
        # or "": chunk cuối có content = None, cộng None vào chuỗi = error -> đổi None -> "" để tránh lỗi
        #end = "": in liền 1 dòng, không xuống dòng sau mỗi chunk
        #flush=True: in ngay, không bị Py giữ lại trong bộ đệm (buffer)
        #print(): xuống dòng sau khi in xong phản hồi

        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]
        #Một lượt gồm 2 message (1 user và 1 assistant), 
        #-> 3 lượt là 6 message. history[-6:] giữ lại 6 phần tử cuối.
        #Phải cắt vì mỗi lượt đều gửi lại toàn bộ history lên API;
        #không cắt thì history dài mãi, tốn token input và chi phí tăng dần.
