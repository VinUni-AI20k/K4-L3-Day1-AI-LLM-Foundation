...
PRICING_PER_1K_TOKENS = {
    "groq/compound": {"input": 0.0025, "output": 0.010},
    "groq/compound-mini": {"input": 0.00015, "output": 0.0006},
}
...
OPENAI_MODEL = os.getenv("LAB_MODEL", "groq/compound")
OPENAI_MINI_MODEL = os.getenv("LAB_MINI_MODEL", "groq/compound-mini")
...
def call_openai(
    ...
    Args:
        ...
        model:       Model OpenAI sử dụng (mặc định: groq/compound).
...
# ---------------------------------------------------------------------------
# Task 1.2 — Gọi groq/compound-mini
# ---------------------------------------------------------------------------
def call_openai_mini(
    ...
    """
    Gọi API với model groq/compound-mini — nhanh hơn và rẻ hơn.
...
# ---------------------------------------------------------------------------
# Task 1.3 — So sánh groq/compound vs groq/compound-mini
# ---------------------------------------------------------------------------
def compare_models(prompt: str) -> dict:
    ...
    Gợi ý:
        cost = (len(response.split()) / 0.75) / 1000 \\
               * PRICING_PER_1K_TOKENS["groq/compound"]["output"]
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    Gợi ý:
        pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["groq/compound"])
        input_cost = input_tokens / 1000 * pricing["input"]
        (.get với fallback: model không có trong bảng giá — ví dụ model NIM
         miễn phí — thì lấy giá groq/compound làm tham chiếu học tập)
    """
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    Cột: Prompt | groq/compound Response | Mini Response | groq/compound Latency | Mini Latency
